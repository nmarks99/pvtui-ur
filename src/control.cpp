...
Control::Control(pvtui::App& app, const std::string& prefix) : prefix(prefix),
    ...
    axis10(Make<ControlAxis>(app, prefix, "PoseRx", "Receive:PoseRx")),
    axis11(Make<ControlAxis>(app, prefix, "PoseRy", "Receive:PoseRy")),
    axis12(Make<ControlAxis>(app, prefix, "PoseRz", "Receive:PoseRz")),
    ...
    tcp_offset_roll(app, prefix + "Control:TCPOffset_Rx.VAL", PVPutType::Double),
    tcp_offset_pitch(app, prefix + "Control:TCPOffset_Ry.VAL", PVPutType::Double),
    tcp_offset_yaw(app, prefix + "Control:TCPOffset_Rz.VAL", PVPutType::Double),
    ...
    speed_slider(app, prefix + "IO:SpeedSlider.VAL", {0.0, 1.0, 0.05}, Color::RGB(179, 194, 227), Color::GrayDark)
{
    ...
    auto tcp_offset_container = Container::Horizontal({
        ...
        return hbox({
            ...
            separatorEmpty(),
            make_offset_entry(tcp_offset_roll, "Rx"),
            separatorEmpty(),
            make_offset_entry(tcp_offset_pitch, "Ry"),
            separatorEmpty(),
            make_offset_entry(tcp_offset_yaw, "Rz"),
            vbox({
                separatorEmpty(),
                text(" mm, rad"),
            }),
